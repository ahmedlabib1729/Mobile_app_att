...
class OdooService {
    ...
    try {
      ...
      if (result.containsKey('result') &&
        ...
        currentEmployee = Employee(
          ...
          avatar128: _buildImageUrl(employeeData['id'], 'avatar_128'),
          image1920: _buildImageUrl(employeeData['id'], 'image_1920'),
          imageUrl: _buildImageUrl(employeeData['id'], 'avatar_128'), // للتوافق مع النموذج القديم
        );

        // حفظ الصورة محلياً للاستخدام بدون إنترنت
        _cacheEmployeeImage(employeeId!);
      ...
      try {
        ...
        if (result['result'] != null && result['result'].isNotEmpty) {
          ...
          currentEmployee = Employee(
            ...
            avatar128: _buildImageUrl(employeeData['id'], 'avatar_128'),
            image1920: _buildImageUrl(employeeData['id'], 'image_1920'),
            imageUrl: _buildImageUrl(employeeData['id'], 'avatar_128'),
          );

          // حفظ الصورة محلياً
          _cacheEmployeeImage(employeeData['id']);
  ...
  // حفظ الصورة محلياً (دالة خاصة)
  Future<void> _cacheEmployeeImage(int employeeId) async {
      ...
      if (result.containsKey('result') && result['result'] is List) {
        ...
        for (var item in result['result']) {
          leaveRequests.add(LeaveRequest(
            ...
            leaveTypeId: item['holiday_status_id'] != false ? item['holiday_status_id'][0] : 0,
            leaveTypeName: item['holiday_status_id'] != false ? item['holiday_status_id'][1] : '',
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'params': {
            ...
            'args': [
              [['active', '=', true]],
              ['id', 'name', 'time_type', 'allocation_type']
            ],
            'kwargs': {},
          },
          'id': DateTime.now().millisecondsSinceEpoch
      ...
      if (result.containsKey('result') && result['result'] is List) {
        ...
        for (var item in result['result']) {
          leaveTypes.add(LeaveType(
            ...
            allocationType: item['allocation_type'] ?? 'no',
          ));
        }
        return leaveTypes;
      } else {
        throw Exception('خطأ في جلب أنواع الإجازات');
      }
    } catch (e) {
