...
class unit_release(models.Model):
    ...
    def action_confirm(self):
        ...
        refund_account=refund_journal.default_account_id.id
        cash_account=cash_journal.default_account_id.id
    ...
    def action_contract(self):
        for release_unit_obj in self:
            ...
            for line in contract_obj.loan_line:
                #if not line.paid:
                loan_lines.append((0,0,{'total_paid_amount': line.total_paid_amount, 'amount':line.amount,'date': line.date,'name':line.name}))
            ...
            res_id  = self.env.ref('itsys_real_estate.ownership_contract_form_view').id
