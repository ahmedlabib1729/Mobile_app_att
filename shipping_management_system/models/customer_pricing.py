...
class ShipmentOrder(models.Model):
    ...
    def _compute_company_cod_fee(self):
        """حساب رسوم COD للشركة (الربح) بناءً على فئة العميل"""
        for record in self:
            ...
            if base_amount < category.company_cod_minimum_amount:
                record.company_cod_fee_amount = 0
                record.company_cod_calculation_details = (
                    f'COD amount ({base_amount:.2f}) is below minimum ({category.company_cod_minimum_amount:.2f})'
                )
                continue

            # حساب الرسوم حسب نوع الدفع
            if record.cod_payment_type == 'visa':
                percentage = category.company_cod_visa_percentage
                fixed_fee = category.company_cod_visa_fixed_fee
            else:  # cash
                percentage = category.company_cod_cash_percentage
                fixed_fee = category.company_cod_cash_fixed_fee
            ...
            if fixed_fee:
                details.append(f"Fixed Fee: {fixed_fee:.2f} EGP")
