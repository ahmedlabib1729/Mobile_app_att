...
class ShipmentOrder(models.Model):
    ...
    @api.depends('sender_id.price_category_id', 'recipient_governorate_id', 'total_weight',
                 ...
                 'total_additional_fees')  # أضف total_additional_fees هنا
        ...
        for record in self:
            if record.customer_category_id and record.recipient_governorate_id:
                ...
                if cost_config:
                    ...
                    # === التعديل هنا: إضافة Total Additional Fees ===
                    record.total_company_cost = (
                            ...
                            record.total_additional_fees  # إضافة الرسوم الإضافية
                    )
                else:
                    # نفس الشيء للحالة الافتراضية
                ...
                # نفس الشيء للحالة بدون category
                record.company_base_cost = 0
    ...
    @api.depends('payment_method', 'cod_amount', 'cod_payment_type', 'customer_category_id', 'shipping_cost', 'cod_amount_sheet_excel')
    def _compute_company_cod_fee(self):
        ...
        for record in self:
            ...
            # حساب المبلغ الأساسي
            base_amount = record.cod_amount_sheet_excel
