...
class CustomerCategoryGovernoratePrice(models.Model):
    """أسعار المحافظات لكل فئة عملاء - محدث للمحافظات الجديدة"""
...
class ShipmentOrder(models.Model):
    ...
    def _compute_company_costs(self):
        """حساب تكاليف الشركة بناءً على فئة العميل والمحافظة - محدث ليشمل Insurance"""
        for record in self:
            if record.customer_category_id and record.recipient_governorate_id:
                ...
                if cost_config:
                    ...
                    # === التعديل هنا: إضافة Company Insurance Fee ===
                    record.total_company_cost = (
                            ...
                            (
                                record.company_insurance_fee_amount if record.insurance_required else 0) +  # إضافة Insurance
    ...
    def _compute_company_insurance_fee(self):
        """حساب رسوم التأمين للشركة (الربح) بناءً على فئة العميل"""
        for record in self:
            ...
            # حساب رسوم التأمين باستخدام دالة الفئة
            ...
            # إضافة معلومات عن رسوم شركة الشحن إذا موجودة
    ...
    def _compute_company_cod_fee(self):
        ...
        for record in self:
            ...
            details.append(f"Payment Type: {payment_type.upper()}")  # هنا التعديل
    ...
    def _compute_customer_pricing(self):
        """حساب السعر النهائي مع الخصم على كل تكاليف الشركة"""
        for record in self:
            # 1. المجموع قبل الخصم (تكلفة الشركة + تكلفة شركة الشحن)
            record.subtotal_before_discount = record.total_company_cost + record.shipping_cost

            # 2. المبلغ القابل للخصم = total_company_cost فقط
    ...
    def get_cost_breakdown(self):
        """الحصول على تفاصيل التكاليف للعرض في الفاتورة - محدث ليشمل Insurance"""
        self.ensure_one()
        return {
            'company_costs': {
                ...
                'company_insurance_fee': self.company_insurance_fee_amount if self.insurance_required else 0,  # إضافة
            ...
            'shipping_costs': {
                ...
                'insurance_fee': self.insurance_fee_amount,  # رسوم التأمين لشركة الشحن
