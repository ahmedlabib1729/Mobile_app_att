...
class ShippingCompany(models.Model):
    ...
    cod_commission_rate = fields.Float(
        string='COD Commission Rate (%)',
        default=1.5,
        help='Percentage of COD amount charged as commission'
    )

    cod_fixed_fee = fields.Float(
        string='COD Fixed Fee',
        default=0,
        help='Fixed fee for COD service (in addition to percentage)'
    )
...
class ShippingCompanyService(models.Model):
    ...
    partner_id = fields.Many2one(
        ...
        help='Linked partner for accounting purposes'
    )
    ...
    @api.onchange('vendor_shipping_cost')
    def _onchange_calculate_our_price(self):
        if self.vendor_shipping_cost and self.shipping_company_id:
            markup = self.shipping_company_id.default_markup_percentage or 30
            self.our_shipping_charge = self.vendor_shipping_cost * (1 + markup / 100)
