...
class ShippingCompany(models.Model):
    ...
    def calculate_insurance_fee(self, product_value, apply_insurance=True):
    ...
    def calculate_cod_fee(self, cod_amount, payment_type='cash', include_shipping_cost=False, shipping_cost=0):
        ...
        # حساب المبلغ الأساسي لـ COD
        base_amount = cod_amount
        if self.cod_include_shipping and include_shipping_cost:
            base_amount += shipping_cost

        # التحقق من الحد الأدنى
        if base_amount < self.cod_minimum_amount:
            return {
                'fee_amount': 0,
                'percentage_used': 0,
                'fixed_fee_used': 0,
                'total_cod_amount': base_amount,
                'reason': f'COD amount ({base_amount:.2f}) is below minimum ({self.cod_minimum_amount:.2f})'
            }

        # حساب الرسوم حسب نوع الدفع
        if payment_type == 'visa':
            percentage = self.cod_visa_percentage
            fixed_fee = self.cod_visa_fixed_fee
        else:  # cash
            percentage = self.cod_cash_percentage
            fixed_fee = self.cod_cash_fixed_fee
    ...
    def _create_temp_price_config(self, governorate_id, zone):
        ...
        return self.env['shipping.governorate.price'].new({
            ...
            'price_per_kg': self.default_price_per_kg,  # استخدام القيمة من الشركة بدلاً من 5
