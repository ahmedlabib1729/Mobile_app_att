...
class ShipmentOrder(models.Model):
    ...
    ], string='Goods Type', default='package')
    ...
    tracking_number_shipping_company = fields.Char(string='shipping Company Tracking Number', tracking=True, copy=False, readonly=True, index=True)
    ...
    @api.depends('state', 'shipping_company_id', 'shipping_cost')
        ...
        for record in self:
            if record.state == 'returned' and record.shipping_company_id:
                if record.shipping_company_id.return_penalty_enabled:
                    ...
                    record.return_penalty_amount = record.shipping_cost * penalty_rate
                else:
    ...
    def _onchange_calculate_shipping(self):
        ...
        if self.payment_method == 'cod':
            ...
            self.cod_amount = self.total_value + company_price

    # في class ShipmentOrder، أضف هذه الدالة المحسوبة:

    @api.depends('total_value', 'total_company_cost', 'total_additional_fees', 'discount_amount', 'payment_method')
    def _compute_cod_amount(self):
        ...
        for record in self:
            if record.payment_method == 'cod':
                ...
                record.cod_amount = record.total_value + company_price
            else:
                record.cod_amount = 0
    ...
    def action_to_torood_hub(self):
        for record in self:
            ...
            record.message_post(
                body="Shipment arrived at Torood Hub",
    ...
    def action_in_transit(self):
        for record in self:
            ...
            record.message_post(
                body=f"Shipment in transit from Torood Hub. Track at: {record.tracking_url or record.tracking_number}",
                subject="In Transit"
            )
        return True
    ...
    def _create_return_customer_invoice(self):
        """إنشاء فاتورة عميل للمرتجع مع الغرامة"""
        self.ensure_one()
        ...
        # حساب المبالغ
        base_amount = self.total_company_cost - self.discount_amount
        penalty_amount = base_amount * (self.shipping_company_id.return_penalty_percentage / 100)
        total_amount = base_amount + penalty_amount

        # إنشاء سطور الفاتورة
        invoice_lines = []

        # السطر الأول: التكلفة الأساسية
        ...
        invoice_lines.append((0, 0, {
            'name': f"Return Penalty ({self.shipping_company_id.return_penalty_percentage:.0f}%)\n"
                    "Additional charges for returned shipment",
            'quantity': 1,
        ...
        invoice_vals = {
            ...
            'narration': f"Return Invoice for Shipment {self.order_number}\n"
                         ...
                         f"Return penalty applied: {penalty_amount:.2f} EGP",
    ...
    def _create_return_vendor_bill(self):
        """إنشاء فاتورة مورد للمرتجع مع الغرامة"""
        self.ensure_one()
        ...
        # حساب المبالغ
        base_amount = self.shipping_cost
        penalty_amount = base_amount * (self.shipping_company_id.return_penalty_percentage / 100)
        total_amount = base_amount + penalty_amount
        ...
        vendor_bill_vals = {
            ...
            'narration': f"Return Bill for Shipment {self.order_number}\n"
                         f"Return penalty: {penalty_amount:.2f} EGP",
            'invoice_line_ids': invoice_lines,
        }
