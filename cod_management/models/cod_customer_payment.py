...
class CODCustomerPayment(models.Model):
    ...
    def action_mark_paid(self):
        ...
        <b>COD Settlement Complete</b><br/>
        Customer: {self.customer_id.name}<br/>
        Total COD: {self.total_cod:.2f} EGP<br/>
        Total Deductions: {self.total_deductions:.2f} EGP<br/>
        Net Amount: {self.net_amount:.2f} EGP<br/>
        """
        ...
        return {
            ...
            'params': {
                'title': _('Success'),
                'message': _(f'Settlement complete. {len(customer_invoices)} invoices paid automatically.'),
                'type': 'success',
                'sticky': True,
            }
        }
