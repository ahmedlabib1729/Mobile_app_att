...
class ShipmentOrderCOD(models.Model):
    ...
    @api.depends('cod_amount_sheet_excel', 'total_company_cost', 'shipping_cost')  # تغيير cod_amount
    def _compute_cod_amounts(self):
        """حساب المبالغ الصافية - الحسبة الصحيحة"""
        for record in self:
            if record.payment_method == 'cod':
                # ما نستلمه من شركة الشحن = COD - تكلفة الشحن
                amount_from_shipping = record.cod_amount_sheet_excel - record.shipping_cost  # هنا التغيير

                # ربحنا الصافي = تكلفتنا - تكلفة الشحن
                our_profit = record.total_company_cost - record.shipping_cost

                # الخصومات من العميل = ربحنا الصافي
                record.total_deductions = our_profit if our_profit > 0 else 0

                # المبلغ الصافي للعميل = ما نستلمه من شركة الشحن - ربحنا
        ...
        for record in self:
            if record.payment_method == 'cod':
                ...
                record.message_post(
                    ...
                    Amount: {record.cod_amount_sheet_excel:.2f} EGP<br/>  # هنا التغيير
