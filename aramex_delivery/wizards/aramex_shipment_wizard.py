...
class AramexShipmentWizard(models.TransientModel):
    ...
    def _prepare_shipment_data(self):
        ...
        shipment = {
            ...
            'OperationsInstructions': '',  # مطلوب حسب WSDL
            'AccountingInstrcutions': '',  # مطلوب حسب WSDL (لاحظ الخطأ المطبعي في WSDL)
            'Details': {
                'Dimensions': {
                ...
                # ChargeableWeight يحسبها Aramex تلقائياً، لكن نرسلها للاحتياط
                'ChargeableWeight': {
                    'Unit': self.weight_unit,
                    'Value': round(self.actual_weight, 2)
                },
                ...
                # CustomsValueAmount هنا قبل COD
                'CustomsValueAmount': {
                    'CurrencyCode': currency_code,
                    'Value': round(declared_value, 2)
                } if declared_value > 0 else None,
                # CashOnDeliveryAmount
                'CashOnDeliveryAmount': {
                    'CurrencyCode': self.cod_currency_id.name if self.cod_currency_id else 'AED',
                    'Value': round(float(self.cod_amount), 2)
                } if self.is_cod and self.cod_amount and float(self.cod_amount) > 0 else None,
                # باقي العناصر بالترتيب
                'InsuranceAmount': {
                    'CurrencyCode': self.insurance_currency.name if self.insurance_currency else 'AED',
                    'Value': round(self.insurance_amount, 2)
                } if hasattr(self,
                             'insurance_amount') and self.insurance_amount and self.insurance_amount > 0 else None,
                'CashAdditionalAmount': None,  # اختياري
                'CashAdditionalAmountDescription': None,  # اختياري
                'CollectAmount': None,  # اختياري
                'Services': ','.join(services_list) if services_list else '',
                'Items': items  # يجب أن يكون آخر عنصر في Details
            }
        }
    ...
    # في aramex_shipment_wizard.py - تعديل _prepare_consignee_address

    def _prepare_consignee_address(self):
        """Prepare consignee address data - Final version with all required fields"""
        partner = self.partner_id
        ...
        if self.aramex_config_id.debug_mode:
            _logger.info("=" * 60)
            _logger.info("Consignee Name Final Check:")
            _logger.info(f"  Name: '{consignee_name}'")
            _logger.info(f"  Length: {len(consignee_name)}")
            _logger.info(f"  Type: {type(consignee_name)}")
            _logger.info("=" * 60)

        # بناء dictionary البيانات - مع إضافة Reference fields
        data = {
            # إضافة Reference fields (قد تكون مطلوبة)
            'Reference1': '',  # أو partner.ref if partner.ref else ''
            'Reference2': '',
            # AccountNumber للـ Consignee (عادة فارغ)
            'AccountNumber': '',
            'PartyAddress': {
                'Line1': (partner.street or 'Address')[:100],
                'Line2': (partner.street2 or '')[:100],
                'Line3': '',
                'City': (partner.city or 'Dubai')[:50],
                'StateOrProvinceCode': (partner.state_id.code if partner.state_id else '')[:50],
                'PostCode': self._validate_and_fix_postal_code(partner.zip,
                                                               partner.country_id.code if partner.country_id else 'AE')[
                    :20],
                'CountryCode': partner.country_id.code if partner.country_id else 'AE'
            },
            'Contact': {
                'Department': '',  # إضافة Department (اختياري)
                'PersonName': consignee_name[:50],
                'Title': '',  # إضافة Title (اختياري)
                'CompanyName': (partner.parent_id.name[:50] if partner.parent_id else '') or '',
                'PhoneNumber1': phone1 or '',
                'PhoneNumber1Ext': '',
                'PhoneNumber2': '',  # مطلوب حتى لو فارغ
                'PhoneNumber2Ext': '',
                'FaxNumber': '',
                'CellPhone': cell_phone or phone1 or '',
        ...
        if self.aramex_config_id.debug_mode:
            _logger.info("Final Consignee Data with References:")
