...
class AramexShipmentWizard(models.TransientModel):
    ...
    def _compute_products_summary(self):
        """حساب ملخص المنتجات للعرض - بدون Internal Reference"""
        for wizard in self:
            if wizard.picking_id:
                ...
                for move in wizard.picking_id.move_ids:
                    ...
                    if qty > 0:
                        # استخدم اسم المنتج فقط بدون SKU
                        product_name = move.product_id.name
                        lines.append(f"{int(qty)} × {product_name}")
    ...
    def _prepare_shipment_data(self):
        """Prepare data for Aramex API - Same description in both fields"""
        self.ensure_one()
        ...
        # إعداد وصف تفصيلي للمنتجات مع الأعداد
        detailed_items_description = []

        for move in picking.move_ids:
            qty = move.quantity if picking.state == 'done' else move.product_uom_qty
            if qty > 0:
                # استخدم اسم المنتج فقط بدون Internal Reference
                product_name = move.product_id.name

                # الوصف التفصيلي: العدد × اسم المنتج
                item_desc = f"{int(qty)} × {product_name}"
                detailed_items_description.append(item_desc)

        # دمج الوصف التفصيلي بعلامة +
        full_description = ' + '.join(
            detailed_items_description) if detailed_items_description else self.description_of_goods

        # استخدم نفس الوصف الكامل في Description of Goods
        # مع التأكد من أنه لا يتجاوز 100 حرف (حد Aramex)
        description_of_goods = full_description
        if len(description_of_goods) > 100:
            # إذا كان طويل جداً، اقطعه واضف ...
            description_of_goods = description_of_goods[:97] + '...'

        # إعداد Comments/Remarks
        remarks = []

        # أضف الوصف الكامل للمنتجات
        if detailed_items_description:
            remarks.append(full_description)

        # أضف التعليقات الإضافية إن وجدت
        if self.comments:
            remarks.append(self.comments)

        # أضف معلومات COD إن وجدت
        ...
        for move in picking.move_ids:
            qty = move.quantity if picking.state == 'done' else move.product_uom_qty
            if qty > 0:
                # استخدم اسم المنتج فقط في item description
                ...
                items.append({
                    ...
                    'Comments': item_description[:50],  # اسم المنتج فقط
        ...
        details = {
            ...
            'DescriptionOfGoods': description_of_goods[:100],  # نفس الوصف الكامل مع الأعداد
        ...
        if self.aramex_config_id.debug_mode:
            _logger.info("=" * 60)
            _logger.info("Aramex Shipment Data Preview:")
            _logger.info("=" * 60)
            _logger.info(f"Description of Goods: {description_of_goods}")
            _logger.info(f"Comments/Remarks: {final_remarks}")
            _logger.info("-" * 60)
            _logger.info("Items Detail:")
            for idx, item in enumerate(items, 1):
                _logger.info(
                    f"  Item {idx}: {item['Comments']} - Qty: {item['Quantity']} - Weight: {item['Weight']['Value']} {item['Weight']['Unit']}")
            _logger.info("=" * 60)
            _logger.info("Full Request JSON:")
    ...
    def _prepare_shipper_address(self):
        """Prepare shipper address data with Sale Order reference - FIXED"""

        # الحصول على رقم Sale Order
        sale_order_ref = ''
        if self.picking_id and self.picking_id.sale_id:
            sale_order_ref = self.picking_id.sale_id.name
        ...
        if self.use_warehouse_address:
            ...
            street_address = partner.street or 'Main Street'
            street2 = partner.street2 or ''
            # مهم جداً: تأكد من وجود City دائماً
            city = partner.city or self.company_id.partner_id.city or 'Dubai'
            ...
            if self.shipper_address:
                ...
                street_address = address_lines[0] if len(address_lines) > 0 else 'Main Street'
                street2 = address_lines[1] if len(address_lines) > 1 else ''
                # استخراج City من العنوان أو استخدام قيمة افتراضية
                ...
                street2 = ''
                city = 'Dubai'  # قيمة افتراضية دائماً
                state_code = ''
                zip_code = ''

            country_code = self.company_id.country_id.code if self.company_id.country_id else 'AE'

        # تأكد من أن City ليست فارغة أبداً
        if not city or city.strip() == '':
            city = 'Dubai'  # قيمة افتراضية

        # بناء البيانات
        data = {
            'Reference1': sale_order_ref[:50],
            'Reference2': '',
            'AccountNumber': self.aramex_config_id.account_number,
            'PartyAddress': {
                'Line1': street_address[:50] if street_address else 'Main Street',
                'Line2': street2[:50] if street2 else '',
                'Line3': '',
                'City': city[:50],  # دائماً سيكون لها قيمة
        ...
        if self.aramex_config_id.debug_mode:
            _logger.info(f"Shipper Data - City: '{data['PartyAddress']['City']}'")
    ...
    def _process_shipment_response(self, response):
        ...
        # Create shipment record - تم إزالة shipper_email
        shipment_vals = {
        ...
        # Add shipper info - بدون shipper_email
        ...
        # Add consignee info - بدون consignee_email
        ...
        shipment_vals.update({
            ...
            # تم إزالة consignee_email من هنا
