...
class AramexConfig(models.Model):
    ...
    def _prepare_transaction(self, reference1='', reference2=''):
        """Prepare Transaction structure for API requests - with all 5 required fields"""
        return {
            'Reference1': reference1 or '',
            'Reference2': reference2 or '',
            'Reference3': '',  # مطلوب حتى لو فارغ
            'Reference4': '',  # مطلوب حتى لو فارغ
            'Reference5': ''  # مطلوب حتى لو فارغ
        }
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" 
    ...
    # في ملف aramex_delivery/models/aramex_config.py
    # استبدل دالة _dict_to_xml_v2 بهذه النسخة النهائية المصححة

    def _dict_to_xml_v2(self, data, parent_tag='', level=0):
        """Convert dictionary to XML string with proper structure - FINAL FIX"""
        xml_parts = []

        # ترتيب العناصر حسب WSDL - مهم جداً!
        element_order = {
            ...
            'Shipper': ['Reference1', 'Reference2', 'AccountNumber', 'PartyAddress', 'Contact'],
            'Consignee': ['Reference1', 'Reference2', 'AccountNumber', 'PartyAddress', 'Contact'],
        ...
        # العناصر المطلوبة دائماً (حتى لو فارغة)
        always_required_elements = {
            'Transaction': ['Reference1', 'Reference2', 'Reference3', 'Reference4', 'Reference5'],
            'Shipment': ['Reference1', 'Reference2', 'Reference3', 'PickupLocation',
                         'OperationsInstructions', 'AccountingInstrcutions'],
            'Shipper': ['Reference1', 'Reference2', 'AccountNumber'],  # هذه مطلوبة في Party
            'Consignee': ['Reference1', 'Reference2', 'AccountNumber'],  # هذه مطلوبة في Party
            'PartyAddress': ['Line1', 'Line2', 'Line3', 'City', 'StateOrProvinceCode', 'PostCode', 'CountryCode'],
        ...
        # عناصر Money الاختيارية
        optional_money_elements = [
            'InsuranceAmount',
            'CashAdditionalAmount',
            'CollectAmount'
        ]

        # عناصر اختيارية أخرى (لا نضيفها إذا لم تكن موجودة)
        truly_optional_elements = [
            'ThirdParty', 'DueDate', 'Attachments', 'ForeignHAWB',
            'TransportType ', 'PickupGUID', 'CashAdditionalAmountDescription',
            'Department', 'Title'
        ]
        ...
        if parent_tag in element_order:
            for key in element_order[parent_tag]:
                ...
                # معالجة خاصة للعناصر المالية الاختيارية
                if key in optional_money_elements:
                    if value is None:
                        continue
                    elif isinstance(value, dict):
                        if not value.get('Value') or value.get('Value') == 0:
                            continue
                        if not value.get('CurrencyCode'):
                            continue

                # تجاهل العناصر الاختيارية تماماً إذا لم تكن موجودة
                elif key in truly_optional_elements:
                ...
                if value is not None:
                    ...
                    # إضافة العناصر المطلوبة الفارغة
                    if parent_tag in always_required_elements and key in always_required_elements[parent_tag]:
