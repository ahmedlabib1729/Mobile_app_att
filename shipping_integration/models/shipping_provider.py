...
class ShippingProvider(models.Model):
    ...
    def create_shipment(self, sale_order):
        """Create shipment for Porter Express - Fixed version with products"""
        self.ensure_one()
        ...
        try:
            ...
            try:
                ...
                for line in sale_order.order_line:
                    # تجاهل الخدمات والمنتجات غير المخزنية
                    if line.product_id and line.product_id.type in ['product', 'consu']:
                        # استخدام default_code (Internal Reference) كـ SKU
                        sku = line.product_id.default_code or line.product_id.barcode or f"PROD_{line.product_id.id}"
                        ...
                        if line.product_id.weight:
                            total_weight += float(line.product_id.weight * line.product_uom_qty)

                        # حساب عدد القطع
                        total_pieces += int(line.product_uom_qty)

                        # حساب قيمة البضائع
                        total_goods_cost += float(line.price_subtotal)

                # إضافة FulfilmentItems إلى request_data إذا كان هناك منتجات
                if fulfilment_items:
                    request_data["FulfilmentItems"] = fulfilment_items

                    # تحديث معلومات الشحنة بناءً على المنتجات الفعلية
                    if total_weight > 0:
                        request_data["ShipmentDetail"]["Weight"] = float(total_weight)
                    if total_pieces > 0:
                        request_data["ShipmentDetail"]["Pieces"] = int(total_pieces)
                    if total_goods_cost > 0:
                        request_data["ShipmentDetail"]["GoodsCost"] = float(total_goods_cost)

                    # تحديث وصف الشحنة بأسماء المنتجات
                    product_names = []
                    order_lines_list = list(sale_order.order_line)
                    for i in range(min(3, len(order_lines_list))):
                        line = order_lines_list[i]
                        if line.product_id and line.product_id.type in ['product', 'consu']:
                            product_names.append(str(line.product_id.name))

                    if product_names:
                        request_data["ShipmentDetail"]["Description1"] = ", ".join(product_names)[:50]
                        if len(sale_order.order_line) > 3:
                            request_data["ShipmentDetail"][
                                "Description2"] = f"and {len(sale_order.order_line) - 3} more items"

            except Exception as e:
                _logger.error(f"Error processing order lines: {str(e)}")
                # Continue without products if there's an error
                pass

            # تحديث COD amount إذا كان مطلوباً
            try:
                cod_amount = sale_order._context.get('shipping_cod_amount', 0)
                if cod_amount > 0:
                    request_data["ShipmentDetail"]["CodAmount"] = float(cod_amount)
                    request_data["ShipmentDetail"]["CodCurrency"] = str(sale_order.currency_id.name or "KWD")
            except Exception as e:
                _logger.error(f"Error setting COD: {str(e)}")
                pass
                    ...
                    if value is not None:
                        ...
                        if mapping.provider_field_path.startswith('Packages.0.'):
                            try:
                                ...
                                if len(request_data.get("Packages", [])) > 0:
                                    if isinstance(request_data["Packages"][0], dict):
                                        request_data["Packages"][0][field_name] = value
                            except Exception as e:
                                _logger.error(f"Error setting package field: {str(e)}")
                                continue
                        else:
                            # Use the helper method to set nested values
                            self._set_value_in_dict_safe(request_data, mapping.provider_field_path, value)
            except Exception as e:
                _logger.error(f"Error applying field mappings: {str(e)}")
                pass
    ...
    def _apply_porter_fixes(self, request_data, sale_order):
        ...
        fixes = {
            ...
            # Collection
            "Collection.PickupDate": "06-Jun-2025",
