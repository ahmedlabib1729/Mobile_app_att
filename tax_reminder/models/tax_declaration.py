...
class TaxClient(models.Model):
    ...
    corporate_tax_settings_ids = fields.One2many('tax.corporate.settings', 'client_id', string='إعدادات ضريبة الشركات')
    company_establishment_date = fields.Date(string='تاريخ تأسيس الشركة', tracking=True)
...
class TaxCorporateSettings(models.Model):
    _name = 'tax.corporate.settings'
    _description = 'إعدادات ضريبة الشركات'
    _rec_name = 'submission_month'

    client_id = fields.Many2one('tax.client', string='العميل', required=True, ondelete='cascade')

    # تاريخ بداية التسجيل في ضريبة الشركات
    tax_registration_start_date = fields.Date(
        string='تاريخ بداية التسجيل الضريبي',
        required=True,
        help='التاريخ الفعلي لبداية التسجيل في ضريبة الشركات'
    )

    # شهر التقديم السنوي
    submission_month = fields.Selection([
        ('1', 'يناير'),
        ('2', 'فبراير'),
        ('3', 'مارس'),
        ('4', 'أبريل'),
        ...
        ('11', 'نوفمبر'),
        ('12', 'ديسمبر')
    ], string='شهر التقديم السنوي', required=True, default='9')

    submission_day = fields.Integer(string='يوم التقديم', default=15,
                                    help='اليوم من الشهر الذي يستحق فيه التقديم')

    # نوع الإقرارات
    declaration_type = fields.Selection([
        ('annual', 'سنوي'),
        ('custom', 'عدد محدد حسب العقد')
    ], string='نوع الإقرارات', default='annual', required=True)

    custom_declaration_count = fields.Integer(string='عدد الإقرارات المخصص',
                                              help='عدد الإقرارات حسب العقد')

    # إعدادات التذكيرات الثلاثة لضريبة الشركات
    enable_corporate_reminders = fields.Boolean(string='تفعيل التذكيرات', default=True)

    # التذكير الأول - بداية السنة
    first_reminder_month = fields.Selection([
        ('1', 'يناير'),
        ('2', 'فبراير'),
        ('3', 'مارس'),
        ('4', 'أبريل'),
        ...
        ('11', 'نوفمبر'),
        ('12', 'ديسمبر')
    ], string='شهر التذكير الأول', default='1',
        help='التذكير الأول في بداية سنة التقديم')
    first_reminder_day = fields.Integer(string='يوم التذكير الأول', default=1)

    # التذكير الثاني - بعد 3 شهور من الأول
    second_reminder_months_after = fields.Integer(
        string='التذكير الثاني بعد (شهور)',
        default=3,
        help='عدد الشهور بعد التذكير الأول'
    )

    # التذكير الثالث - قبل شهر من التقديم
    third_reminder_months_before = fields.Integer(
        string='التذكير الثالث قبل التقديم (شهور)',
        default=1,
        help='عدد الشهور قبل موعد التقديم'
    )
    enable_reminder = fields.Boolean(string='تفعيل التذكيرات', default=True)
    # حقول محسوبة
    first_declaration_start_date = fields.Date(string='بداية فترة الإقرار الأول',
                                               compute='_compute_first_declaration_dates', store=True)
    first_declaration_end_date = fields.Date(string='نهاية فترة الإقرار الأول',
                                             compute='_compute_first_declaration_dates', store=True)
    first_declaration_due_date = fields.Date(string='تاريخ استحقاق الإقرار الأول',
                                             compute='_compute_first_declaration_dates', store=True)
    days_before_reminder = fields.Integer(
        string='أيام التذكير قبل الاستحقاق',
        default=30,
        help='عدد الأيام قبل موعد الاستحقاق لإرسال التذكير'
    )
    active = fields.Boolean(string='نشط', default=True)
    last_generated_year = fields.Integer(string='آخر سنة تم توليدها')

    @api.depends('tax_registration_start_date', 'submission_month', 'submission_day')
    def _compute_first_declaration_dates(self):
        """حساب تواريخ الإقرار الأول"""
        for record in self:
            if not record.tax_registration_start_date or not record.submission_month:
                record.first_declaration_start_date = False
                record.first_declaration_end_date = False
                record.first_declaration_due_date = False
                continue

            start_date = record.tax_registration_start_date
            submission_month = int(record.submission_month)

            # بداية الفترة = تاريخ بداية التسجيل الضريبي
            record.first_declaration_start_date = start_date

            # نهاية الفترة = نهاية السنة التالية لسنة البداية
            end_year = start_date.year + 1
            record.first_declaration_end_date = datetime(end_year, 12, 31).date()

            # تاريخ الاستحقاق = شهر التقديم من السنة التي تلي نهاية الفترة
            due_year = end_year + 1
            try:
                record.first_declaration_due_date = datetime(due_year, submission_month, record.submission_day).date()
            except ValueError:
                # في حالة يوم غير صحيح، نأخذ آخر يوم في الشهر
                import calendar
                last_day = calendar.monthrange(due_year, submission_month)[1]
                record.first_declaration_due_date = datetime(due_year, submission_month,
                                                             min(record.submission_day, last_day)).date()

    def _calculate_reminder_dates(self, due_date):
        """حساب تواريخ التذكيرات الثلاثة لإقرار معين"""
        from dateutil.relativedelta import relativedelta

        due_year = due_date.year
        submission_month = int(self.submission_month)

        # التذكير الأول: أول الشهر الأول في سنة التقديم
        first_reminder_month = int(self.first_reminder_month) if self.first_reminder_month else 1
        first_reminder = datetime(due_year, first_reminder_month, self.first_reminder_day).date()

        # التذكير الثاني: بعد 3 شهور من التذكير الأول
        second_reminder = first_reminder + relativedelta(months=self.second_reminder_months_after)

        # التذكير الثالث: قبل شهر من موعد التقديم
        third_reminder = due_date - relativedelta(months=self.third_reminder_months_before)
    ...
    def generate_corporate_tax_declarations(self):
        """توليد إقرارات ضريبة الشركات"""
        self.ensure_one()

        if not self.tax_registration_start_date:
            raise UserError('يجب تحديد تاريخ بداية التسجيل الضريبي أولاً')

        if not self.client_id.has_corporate_tax:
            raise UserError('يجب تفعيل ضريبة الشركات للعميل أولاً')

        declarations_to_create = []
        start_date = self.tax_registration_start_date
        submission_month = int(self.submission_month)
        today = fields.Date.today()

        # الإقرار الأول - يغطي من بداية التسجيل حتى نهاية السنة التالية
        first_period_start = start_date
        first_period_end = datetime(start_date.year + 1, 12, 31).date()
        first_due_year = start_date.year + 2

        try:
            first_due_date = datetime(first_due_year, submission_month, self.submission_day).date()
        except ValueError:
            import calendar
            last_day = calendar.monthrange(first_due_year, submission_month)[1]
            first_due_date = datetime(first_due_year, submission_month, min(self.submission_day, last_day)).date()

        # حساب تواريخ التذكيرات للإقرار الأول
        reminder_dates = self._calculate_reminder_dates(first_due_date)

        # التحقق من عدم وجود إقرار أول موجود
        existing_first = self.env['tax.declaration'].search([
            ('client_id', '=', self.client_id.id),
            ('tax_type', '=', '9%'),
            ('period_start_date', '=', first_period_start),
            ('period_end_date', '=', first_period_end),
            ('state', '!=', 'cancelled')
        ], limit=1)
        ...
        if not existing_first:
            declarations_to_create.append({
                ...
                'tax_type': '9%',
                'period_type': 'annual',
                'period': '1',  # الإقرار الأول
                'year': first_period_end.year,
                'period_start_date': first_period_start,
                'period_end_date': first_period_end,
        ...
        # الإقرارات التالية - كل سنة مالية كاملة
        if self.declaration_type == 'annual':
            years_to_generate = 5
        else:
            years_to_generate = self.custom_declaration_count - 1  # ناقص الإقرار الأول

        for i in range(1, years_to_generate + 1):
            year_start = datetime(first_period_end.year + i, 1, 1).date()
            year_end = datetime(first_period_end.year + i, 12, 31).date()
            due_year = first_period_end.year + i + 1

            try:
                due_date = datetime(due_year, submission_month, self.submission_day).date()
            except ValueError:
                import calendar
                last_day = calendar.monthrange(due_year, submission_month)[1]
                due_date = datetime(due_year, submission_month, min(self.submission_day, last_day)).date()

            # حساب تواريخ التذكيرات لكل إقرار
            reminder_dates = self._calculate_reminder_dates(due_date)

            # التحقق من عدم وجود إقرار موجود
            existing = self.env['tax.declaration'].search([
                ('client_id', '=', self.client_id.id),
                ('tax_type', '=', '9%'),
        ...
        if declarations_to_create:
            ...
            if created:
                self.last_generated_year = declarations_to_create[-1]['year']

                self.client_id.message_post(
                    body=f"تم توليد {len(created)} إقرار ضريبة شركات (9%)"
                )

        return len(declarations_to_create)