...
class EmiratesPostShipmentWizard(models.TransientModel):
    ...
    # Receiver Details - Modified for international shipping
    ...
    def _compute_is_domestic_shipment(self):
        ...
        for wizard in self:
            wizard.is_domestic_shipment = wizard.receiver_country_id == uae_country
    ...
    def _onchange_partner_id(self):
        if self.partner_id:
            ...
            # Set country from partner
            if self.partner_id.country_id:
                self.receiver_country_id = self.partner_id.country_id

            # Set state if available
            if self.partner_id.state_id:
                self.receiver_state_id = self.partner_id.state_id
            ...
            if self.partner_id.city:
                ...
                # Set city name for international shipments
                self.receiver_city_name = self.partner_id.city
            self.receiver_address = ', '.join(address_parts)

    @api.onchange('receiver_country_id')
    def _onchange_receiver_country_id(self):
        """Clear location fields when country changes"""
        self.receiver_state_id = False
        self.receiver_emirate_id = False
        # Don't clear city name if it was populated from partner
        if not self.partner_id or not self.partner_id.city:
    ...
    def _get_receiver_city_for_api(self):
        """Get appropriate city value for API"""
        if self.is_domestic_shipment and self.receiver_emirate_id:
            return int(self.receiver_emirate_id.emirate_id)
        else:
            # For international, return city name
            return self.receiver_city_name or 'N/A'

    def _get_receiver_state_for_api(self):
        """Get appropriate state/emirate name for API"""
        if self.is_domestic_shipment and self.receiver_emirate_id:
            return self.receiver_emirate_id.name
        elif self.receiver_state_id:
            return self.receiver_state_id.name
        else:
            return self.receiver_city_name or 'N/A'
    ...
    def action_create_shipment(self):
        ...
        if config.debug_mode:
            ...
            shipment = self.env['emirates.post.shipment'].create({
                ...
                'receiver_state_id': self.receiver_state_id.id if self.receiver_state_id else False,
                'receiver_emirate_id': self.receiver_emirate_id.id if self.receiver_emirate_id else False,
                'receiver_city_name': self.receiver_city_name,
        ...
        booking_data = {
            "BookingRequest": {
                ...
                "SenderEmail": self.sender_email or None,
                "SenderZipCode": "00000",
                ...
                "ReceiverCity": self._get_receiver_city_for_api(),
                "ReceiverContactMobile": self.receiver_mobile,
                "ReceiverContactPhone": self.receiver_phone,
                "ReceiverEmail": self.receiver_email or None,
                "ReceiverZipCode": "00000",
                "ReceiverState": self._get_receiver_state_for_api(),
        ...
        try:
            ...
            # For booking, keep the /genericapi in the path
            ...
            response = requests.post(
                ...
                headers=headers,
                timeout=30,
                verify=False  # Add this for test environment
            )
            ...
            if response.status_code == 200:
                ...
                raise UserError(_(
                    'Emirates Post server is currently unavailable (Error 520).\n\n'
                    'This is a common issue with their test environment.\n'
                    'Please:\n'
                    '1. Enable Debug Mode in configuration to use mock data\n'
                    '2. Try again later\n'
                    '3. Contact Emirates Post for production credentials'
                ))
            else:
                raise UserError(_(
            ...
            if 'Error 520' in str(e):
                raise UserError(_(
                    'Emirates Post server is currently unavailable.\n'
                    'Please enable Debug Mode to use test data.'
                ))
            raise UserError(_(
