...
class CourseSession(models.Model):
    ...
    # Datetime fields for calendar view - هذه الحقول مطلوبة للتقويم
    date_start = fields.Datetime(string='Start', compute='_compute_datetime', store=True)
    date_stop = fields.Datetime(string='End', compute='_compute_datetime', store=True)

    # Alternative names that calendar might look for
    date_delay = fields.Float(string='Duration', compute='_compute_duration', store=True)
    ...
    @api.depends('name', 'course_id', 'date')
    def _compute_display_name(self):
        for record in self:
            if record.course_id and record.date:
                record.display_name = f"{record.course_id.name} - {record.name} - {record.date}"
            else:
                record.display_name = record.name or "Session"
    ...
    def _compute_datetime(self):
        for record in self:
            if record.date:
                ...
                record.date_start = start_dt
                record.date_stop = end_dt
            else:
