...
class QuranStudent(models.Model):
    ...
    certificate_ids = fields.Many2many(
        'ir.attachment',
        'student_certificate_rel',
        'student_id',
        'attachment_id',
        string='الشهادات',
        domain=[('res_model', '=', 'quran.student')],
        help='شهادات الطالب'
    )
    ...
    def action_create_portal_user(self):
        ...
        user_vals = {
            ...
            'password': '1234',  # كلمة المرور الافتراضية = 1234
        }
    ...
    def action_reset_portal_password(self):
        ...
        return {
            ...
            'params': {
                'title': _('نجح'),
                'message': _('تم إعادة تعيين كلمة المرور إلى 1234'),
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def _send_password_reset_email(self):
        ...
        <div style="direction: rtl; text-align: right; font-family: Arial, sans-serif;">
            ...
            <ul>
                <li>البريد الإلكتروني: {self.email}</li>
                <li>كلمة المرور الجديدة: 1234</li>
            </ul>
            <p style="color: red;"><strong>تنبيه:</strong> يُنصح بتغيير كلمة المرور بعد الدخول لضمان أمان حسابك.</p>
            <p>يمكنك الدخول من خلال الرابط: <a href="{self.get_base_url()}/web/login">دخول البورتال</a></p>
        </div>
        """
    ...
    def check_access_rights(self, operation, raise_exception=True):
        """السماح لمستخدم البورتال برؤية بياناته"""
        if operation == 'read' and self.env.user.has_group('base.group_portal'):
            return True
        return super().check_access_rights(operation, raise_exception)

    @api.model
    def check_access_rule(self, operation):
        """السماح لمستخدم البورتال برؤية بياناته فقط"""
        if operation == 'read' and self.env.user.has_group('base.group_portal'):
            # السماح للطالب برؤية سجله فقط
