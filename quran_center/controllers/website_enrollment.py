...
class WebsiteEnrollment(http.Controller):
    ...
    def submit_enrollment(self, **post):
        ...
        required_fields = [
            ...
            'memorization_level',
            'memorization_start_page', 'memorization_end_page',
            'email'
        ]
        ...
        # حساب عدد الصفحات المحفوظة تلقائياً
        try:
            start_page = int(post.get('memorization_start_page', 1))
            end_page = int(post.get('memorization_end_page', 1))
        except ValueError:
            start_page = 1
            end_page = 1
            error['memorization_pages'] = 'invalid'
            error_message.append('قيم الصفحات يجب أن تكون أرقام صحيحة')

        # حساب عدد الصفحات
        if start_page > 0 and end_page > 0:
            if end_page >= start_page:
                calculated_pages = end_page - start_page + 1
            else:
                # إذا كانت صفحة النهاية أقل من البداية، اعتبر أن الحفظ يمتد عبر المصحف
                calculated_pages = start_page - end_page + 1
        else:
            calculated_pages = 0

        # استخدام القيمة المحسوبة بدلاً من القيمة المرسلة
        post['current_memorized_pages'] = str(calculated_pages)

        # التحقق من صحة النطاق
        if start_page < 1 or start_page > 604:
            error['memorization_start_page'] = 'invalid'
            error_message.append('صفحة البداية يجب أن تكون بين 1 و 604')

        if end_page < 1 or end_page > 604:
            error['memorization_end_page'] = 'invalid'
            error_message.append('صفحة النهاية يجب أن تكون بين 1 و 604')

        if calculated_pages > 604:
            error['current_memorized_pages'] = 'invalid'
            error_message.append('عدد الصفحات الإجمالي لا يمكن أن يتجاوز 604 صفحة')

        if calculated_pages == 0:
            error['current_memorized_pages'] = 'invalid'
            error_message.append('يجب تحديد نطاق صحيح للصفحات المحفوظة')
        ...
        try:
            ...
            enrollment_vals = {
                ...
                'current_memorized_pages': calculated_pages,  # استخدام القيمة المحسوبة
                'memorization_level': post.get('memorization_level'),
                'memorization_start_page': start_page,
                'memorization_end_page': end_page,
    ...
    def programs_page(self, **kwargs):
        """عرض صفحة البرامج"""
        return request.render('quran_center.website_programs_template')