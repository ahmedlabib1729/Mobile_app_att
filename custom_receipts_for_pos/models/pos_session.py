...
class PosSession(models.Model):
    ...
    def _pos_data_process(self, loaded_data):
        """Add custom receipt data to POS loaded data"""
        super()._pos_data_process(loaded_data)
        # Add custom receipt configuration
        if self.config_id.is_custom_receipt and self.config_id.receipt_design_id:
            loaded_data['pos.config'][0]['is_custom_receipt'] = self.config_id.is_custom_receipt
            loaded_data['pos.config'][0]['design_receipt'] = self.config_id.design_receipt
            loaded_data['pos.config'][0]['receipt_design_id'] = self.config_id.receipt_design_id.id