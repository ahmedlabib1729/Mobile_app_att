...
class ProgramLevel(models.Model):
    ...
    # Removed price field - now calculated from units
    ...
    total_price = fields.Monetary(string='Total Price', compute='_compute_units_info', store=True)
    ...
    @api.depends('unit_ids', 'unit_ids.number_of_sessions', 'unit_ids.credit_hours', 'unit_ids.price')
    def _compute_units_info(self):
        for record in self:
            ...
            # Calculate total price from units
            record.total_price = sum(record.unit_ids.mapped('price'))

    @api.depends('units_count', 'total_price', 'currency_id')
    def _compute_display_info(self):
        for record in self:
            currency_symbol = record.currency_id.symbol or ''
            record.display_info = f"{record.units_count} Units - {currency_symbol}{record.total_price:,.2f}"
