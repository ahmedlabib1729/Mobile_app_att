...
class ApplicationProgram(models.Model):
    ...
    awarding_body = fields.Text(related='program_id.awarding_body', string='Awarding Body', store=True)
    ...
    def get_formatted_selection(self):
        ...
        if self.program_id.program_type == 'qualification':
            ...
            for level in self.level_ids.sorted('sequence'):
                level_info = f"• {level.name} - {level.currency_id.symbol}{level.price:,.2f}"
                if level.unit_ids:
