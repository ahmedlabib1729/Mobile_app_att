...
class Program(models.Model):
    ...
    training_level = fields.Selection([
        ...
        ('advanced', 'Advanced'),
        ('expert', 'Expert')
    ], string='Training Level', default='beginner')
    ...
    # Training Fields (فقط السعر والوصف وعدد الساعات)
    training_name = fields.Char(string='Training Name', tracking=True)
    training_description = fields.Text(string='Training Description')
    training_price = fields.Monetary(string='Training Price')
    training_duration = fields.Float(string='Duration (Hours)')
    ...
    @api.depends('level_ids', 'level_ids.price', 'program_type', 'training_price')
    def _compute_levels_info(self):
        for record in self:
            if record.program_type == 'qualification':
                ...
                # For training, the total price is the training price itself
                record.levels_count = 0
                record.total_price = record.training_price or 0.0
    ...
    def _onchange_program_type(self):
        if self.program_type == 'training':
            ...
            self.training_description = False
            self.training_price = 0.0
