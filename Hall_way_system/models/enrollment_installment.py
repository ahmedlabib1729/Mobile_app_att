...
class EnrollmentInstallment(models.Model):
    ...
    def action_view_invoice(self):
        ...
        if not self.invoice_id:
            raise UserError(_('No invoice created for this installment yet.'))
    ...
    def action_create_invoice(self):
        ...
        if not self.enrollment_id.partner_id:
            raise UserError(_('Please set customer (partner) for the enrollment first.'))

        # Get education product
        education_product = self.enrollment_id._get_education_product()

        # Create invoice
        invoice_vals = {
            'move_type': 'out_invoice',
            'partner_id': self.enrollment_id.partner_id.id,
            ...
            'invoice_line_ids': [(0, 0, {
                ...
                'name': f"{self.enrollment_id.program_id.display_name} - Installment {self.sequence}",
                'quantity': 1,
