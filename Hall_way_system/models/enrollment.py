...
class Enrollment(models.Model):
    ...
    program_type = fields.Selection(related='program_id.program_type',
                                    string='Program Type', store=True)

    # For Qualification
    level_ids = fields.Many2many('program.level', string='Enrolled Levels',
                                 compute='_compute_program_info', store=True)

    # For Training - حقول إضافية للتدريب
    training_name = fields.Char(related='program_id.training_name',
                                string='Training Name', store=True)
    training_duration = fields.Float(related='program_id.training_duration',
                                     string='Training Duration (Hours)', store=True)
    training_description = fields.Text(related='program_id.training_description',
                                       string='Training Description', store=True)
    ...
    def _compute_program_info(self):
        for record in self:
            if record.application_program_id:
                ...
                # للـ qualification فقط نحتاج المستويات
                if record.application_program_id.program_id.program_type == 'qualification':
                    record.level_ids = record.application_program_id.level_ids
                else:
                    record.level_ids = [(5, 0, 0)]  # Clear all for training
            else:
                record.program_id = False
                record.level_ids = [(5, 0, 0)]  # Clear all

    @api.depends('student_id', 'enrollment_number', 'program_id', 'program_type')
    def _compute_display_name(self):
        for record in self:
            if record.student_id and record.program_id:
                # عرض مختلف حسب نوع البرنامج
                if record.program_type == 'training':
                    record.display_name = f"{record.enrollment_number} - {record.student_id.full_name} - {record.training_name}"
                else:
    ...
    @api.depends('start_date', 'level_ids', 'program_type', 'training_duration')
    def _compute_expected_end_date(self):
        for record in self:
            ...
            if record.program_type == 'training' and record.training_duration:
                # للتدريب: حساب بناءً على عدد الساعات
                # افتراض 4 ساعات يومياً، 5 أيام في الأسبوع
                hours_per_day = 4
                days_per_week = 5
                hours_per_week = hours_per_day * days_per_week

                # حساب عدد الأسابيع المطلوبة
                weeks_needed = record.training_duration / hours_per_week
                days_needed = int(weeks_needed * 7)

                record.expected_end_date = record.start_date + timedelta(days=days_needed)
            elif record.program_type == 'qualification' and record.level_ids:
    ...
    @api.depends('invoice_ids.amount_residual', 'invoice_ids.state', 'total_amount')
    ...
    def action_confirm(self):
        for record in self:
            ...
            # Validate required fields حسب نوع البرنامج
            if record.program_type == 'qualification' and not record.level_ids:
                raise UserError(_('Please select at least one level.'))
            elif record.program_type == 'training' and not record.program_id:
                raise UserError(_('Please select a training program.'))
    ...
    def _create_single_invoice(self):
        ...
        if self.program_type == 'qualification':
            ...
            line_vals = {
                'product_id': education_product.id,
                'name': f"{self.training_name} - {self.training_duration} Hours",
                'quantity': 1,
    ...
    def _create_installment_invoices(self):
        ...
        for installment in self.installment_ids:
            ...
            program_name = self.training_name if self.program_type == 'training' else self.program_id.display_name
            ...
            invoice_vals = {
                ...
                'invoice_line_ids': [(0, 0, {
                    'product_id': education_product.id,
                    'name': f"{program_name} - Installment {installment.sequence}",
                    'quantity': 1,
