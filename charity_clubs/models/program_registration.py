...
class ProgramRegistration(models.Model):
    ...
    student_name = fields.Char(
        string='اسم الطالب (كما في الهوية)',
        required=True,
        tracking=True
    )
    ...
    native_language = fields.Selection([
        ...
        ('hindi', 'الهندية'),
        ('other', 'أخرى')
    ], string='اللغة الأم', required=True)
    ...
    attended_arabic_club = fields.Selection([
        ('yes', 'نعم'),
        ('no', 'لا')
    ], string='هل حضر نادي عربي سابقاً؟', default='no')

    attended_summer_activities = fields.Selection([
        ('yes', 'نعم'),
        ('no', 'لا')
    ], string='هل حضر الأنشطة الصيفية سابقاً؟', default='no')

    interested_quran = fields.Selection([
        ('yes', 'نعم'),
        ('no', 'لا')
    ], string='مهتم بحفظ القرآن؟', default='no')
    ...
    emirates_id_copy = fields.Binary(
        string='نسخة من الهوية الإماراتية',
        required=True
    )
    ...
    terms_accepted = fields.Boolean(
        string='الموافقة على الشروط والأحكام',
        required=True
    )
    ...
    def _check_emirates_id(self):
        ...
        for reg in self:
            if reg.emirates_id:
                # التحقق من أن الرقم يحتوي على 15 رقم (التنسيق: 784-YYYY-XXXXXXX-X)
                id_pattern = re.compile(r'^\d{3}-?\d{4}-?\d{7}-?\d{1}$')
                if not id_pattern.match(reg.emirates_id):
                    raise ValidationError('رقم الهوية الإماراتية غير صحيح! يجب أن يكون بالتنسيق: 784-YYYY-XXXXXXX-X')

    @api.constrains('terms_accepted')
    def _check_terms_accepted(self):
        """التأكد من الموافقة على الشروط"""
        for reg in self:
            if reg.state != 'draft' and not reg.terms_accepted:
                raise ValidationError('يجب الموافقة على الشروط والأحكام!')
